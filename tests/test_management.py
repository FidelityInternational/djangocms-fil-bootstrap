...
class BootstrapCommandTest(TestCase):
    def test_required_parameter(self):
        ...
        self.assertEqual(
            str(context.exception), "Error: the following arguments are required: file"
        )
    ...
    def test_bootstrap_file_not_found(self):
        ...
        with patch(
            ...
            command, "get_file_path", return_value="foo"
        ) as path, patch(
        ...
        self.assertEqual(
            str(context.exception), "Could not open specified file (foo). Aborting."
        )
